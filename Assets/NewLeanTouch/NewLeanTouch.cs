...
namespace NewLean.Touch
{
	...
	// You can access this data via NewLean.Touch.NewLeanTouch.Instance.Fingers, or hook into the NewLean.Touch.NewLeanTouch.On___ events.
	// NOTE: To prevent lag you may want to edit your ScriptExecutionOrder to force this to update before your scripts
	[ExecuteInEditMode]
	[DisallowMultipleComponent]
	public partial class NewLeanTouch : MonoBehaviour
	{   
        // This contains all the active and enabled NewLeanTouch instances
		public static List<NewLeanTouch> Instances = new List<NewLeanTouch>();

		// This list contains all currently active fingers (including simulated ones)
		public static List<NewLeanFinger> Fingers = new List<NewLeanFinger>(10);

		// This list contains all currently inactive fingers (this allows for pooling and tapping)
		public static List<NewLeanFinger> InactiveFingers = new List<NewLeanFinger>(10);

		// This gets fired when a finger begins touching the screen (NewLeanFinger = The current finger)
		public static System.Action<NewLeanFinger> OnFingerDown;

		// This gets fired every frame a finger is touching the screen (NewLeanFinger = The current finger)
		public static System.Action<NewLeanFinger> OnFingerSet;

		// This gets fired when a finger stops touching the screen (NewLeanFinger = The current finger)
		public static System.Action<NewLeanFinger> OnFingerUp;

		// This gets fired when a finger taps the screen (this is when a finger begins and stops touching the screen within the 'TapThreshold' time) (NewLeanFinger = The current finger)
		public static System.Action<NewLeanFinger> OnFingerTap;

		// This gets fired when a finger swipes the screen (this is when a finger begins and stops touching the screen within the 'TapThreshold' time, and also moves more than the 'SwipeThreshold' distance) (NewLeanFinger = The current finger)
		public static System.Action<NewLeanFinger> OnFingerSwipe;

		// This gets fired every frame at least one finger is touching the screen (List = Fingers)
		public static System.Action<List<NewLeanFinger>> OnGesture;
		...
		private static List<NewLeanFinger> filteredFingers = new List<NewLeanFinger>(10);
		...
		public static NewLeanTouch Instance
		{
			get
			{
		...
		{
			// Only run the update methods if this is the first instance (i.e. if your scene has more than one NewLeanTouch component, only use the first)
			if (Instances[0] == this)
			{
		...
		private NewLeanFinger FindFinger(int index)
		{
			for (var i = Fingers.Count - 1; i>= 0; i--)
			{
				var finger = Fingers[i];
